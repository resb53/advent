...
class Jcard:
    ...
    def __new__(cls, value):
        if value in Jcard.order:
            return super().__new__(cls)
        else:
            raise InvalidCardException(f"Jcard must have a value in {Jcard.order}.")
    ...
    def __lt__(self, other):
        return Jcard.order.index(self.value) < Jcard.order.index(other.value)
...
class Jhand:
    ...
    def __new__(cls, value):
        if type(value) is not list:
            raise InvalidHandException("Jhand must be a list of 5 Cards.")
        else:
            if len(value) != 5:
                raise InvalidHandException("Jhand must contain exactly 5 Cards.")
            for crd in value:
                if type(crd) is not Jcard:
                    raise InvalidHandException("Jhand must only contain Cards.")
            return super().__new__(cls)

    def __init__(self, value):
        self.contents = value

        # Determine most powerful hand type
        sorthand = sorted(value)
        groups = []
        unique = []
    ...
    def __lt__(self, other):
        if self.type != other.type:
            return Jhand.types.index(self.type) < Jhand.types.index(other.type)
        else:
            if self.contents[0] != other.contents[0]:
                return self.contents[0] < other.contents[0]
            elif self.contents[1] != other.contents[1]:
...
def parseInput(inp):
    ...
    for line in input_fh:
        ...
        jhand = Jhand([Jcard(x) for x in cards])
